    ...
    public class AuthController : ControllerBaseExtension
    {
        ...
        public async Task<IActionResult> SignUpAsync([FromBody] SignUpRequestDTO signUpRequestDto)
        {
            try
            {
                ...
                {
                    _logger.LogError("signUpRequestDto is null");
                    return BadRequest("Invalid request");
                }

                _logger.LogInformation("Registering user: {Email}", signUpRequestDto.Email);

                var personDom = _mapper.Map<PersonDom>(signUpRequestDto.Person);
                _logger.LogInformation("Mapped personDom: {@PersonDom}", personDom);

                await _authManager.CreateNewPersonAccountAsync(
            ...
            {
                _logger.LogWarning("Password error: {Message}", exception.Message);
                return BadRequest(exception.Message);
            }
            catch (DuplicateEmailException exception)
            {
                _logger.LogWarning("Duplicate email error: {Message}", exception.Message);
                return Conflict(exception.Message);
            }
            catch (Exception exception)
            {
                _logger.LogError(exception, "Unexpected error during registration");
                 return StatusCode(500, new { message = "Registration failed", error = exception.Message });// Optional: return actual message for dev
            }
        }
