...
--
-- Host: localhost    Database: NieuweTechnologie
...
CREATE DATABASE IF NOT EXISTS NieuweTechnologie
;
USE NieuweTechnologie
;
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
/*!50003 CREATE*/  /*!50003 TRIGGER `handle_new_user` BEFORE INSERT ON `person` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
/*!50003 CREATE*/  /*!50003 TRIGGER `after_Person_delete` AFTER DELETE ON `person` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
/*!50003 CREATE*/  /*!50003 TRIGGER `delete_from_Person_details` AFTER DELETE ON `person` FOR EACH ROW BEGIN
    DELETE FROM auth_users WHERE id = OLD.UserId;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
--
DROP TRIGGER IF EXISTS `delete_from_Person_details`;
...
/*!50001 CREATE VIEW `Personview` AS SELECT 
 ...
 1 AS `Email`,
 1 AS `fullname`,
 1 AS `RunningClubId` */;  -- ✅ تمت الإضافة هنا
...
--
-- ------------------------------------------------------
-- Create dedicated user for NieuweTechnologie
-- ------------------------------------------------------
...
-- Grant access only to NieuweTechnologie
GRANT SELECT, INSERT, UPDATE, DELETE, EXECUTE ON NieuweTechnologie
.* TO 'evergemsejogging'@'%';
...
BEGIN
    ...
    SELECT JSON_ARRAYAGG(
        JSON_OBJECT(
            ...
            'zip_code', a.ZipCode,
            'user_role', pr.role,
             'running_club_id', p.RunningClubId
        )
    ) INTO result
    FROM Personview p
    JOIN address a ON a.Id = p.AddressId
...
CREATE PROCEDURE update_user_email(
    IN old_email_address VARCHAR(255),
    IN new_email_address VARCHAR(255)
)
BEGIN
    DECLARE msg VARCHAR(512);

    IF new_email_address NOT REGEXP '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' THEN
        SET msg = CONCAT('Invalid new email: ', new_email_address);
    ...
    IF ROW_COUNT() = 0 THEN
        SET msg = CONCAT('Email address not found: ', old_email_address);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = msg;
    END IF;
END$$
...
-- First, let's verify the tables have the correct columns before adding constraints
-- Only add constraints that reference existing columns

-- Person table constraints (already mostly defined in CREATE TABLE)
ALTER TABLE person
  ADD CONSTRAINT FK_Person_address_AddressId FOREIGN KEY (AddressId) REFERENCES address(AddressId) ON DELETE RESTRICT,
  ADD CONSTRAINT FK_Person_school_SchoolId FOREIGN KEY (SchoolId) REFERENCES school(SchoolId) ON DELETE RESTRICT,
...
-- Competition table constraint
ALTER TABLE Competition
  ADD CONSTRAINT FK_Competition_address_AddressId FOREIGN KEY (AddressId) REFERENCES address(AddressId) ON DELETE RESTRICT;

-- CompetitionPerCategory table constraints
ALTER TABLE CompetitionPerCategory
  ADD CONSTRAINT FK_CompetitionPerCategory_AgeCategory_AgeCategoryId FOREIGN KEY (AgeCategoryId) REFERENCES AgeCategory(Id) ON DELETE RESTRICT,
  ADD CONSTRAINT FK_CompetitionPerCategory_Competition_CompetitionId FOREIGN KEY (CompetitionId) REFERENCES Competition(Id) ON DELETE CASCADE;

-- Registration table constraints
ALTER TABLE Registration
  ADD CONSTRAINT FK_Registration_CompetitionPerCategory_CompetitionPerCategoryId FOREIGN KEY (CompetitionPerCategoryId) REFERENCES CompetitionPerCategory(Id) ON DELETE CASCADE,
  ADD CONSTRAINT FK_Registration_person_PersonId FOREIGN KEY (PersonId) REFERENCES person(Id) ON DELETE CASCADE,
  ADD CONSTRAINT FK_Registration_Competition_CompetitionId FOREIGN KEY (CompetitionId) REFERENCES Competition(Id) ON DELETE CASCADE;